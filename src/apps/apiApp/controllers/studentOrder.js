...
class StudentOrderController extends BaseController {
  ...
  index = async (ctx) => {
    await ctx.$db.studentOrder.find({ where: { uuid: ctx.params.uuid } });
  }

  create = async (ctx) => {
    const { uuid } = ctx.params;
    const { studentOrder, productOrder } = ctx.$db;
    const studentStoreData = await this.getStudentStoreData(ctx.$db, uuid, ctx.request.body.storeUuid);
